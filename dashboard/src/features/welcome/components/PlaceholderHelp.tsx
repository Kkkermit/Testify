...
export function PlaceholderHelp({ onInsert }: { onInsert: (token: string) => void }): React.JSX.Element {
	return (
			...
			{WELCOME_PLACEHOLDERS.map(({ token, describes }) => (
				<Tooltip key={token} label={describes}>
					<button
						...
						className="bg-muted text-muted-foreground hover:text-foreground hover:bg-muted/70 rounded px-2 py-1 font-mono text-xs transition-colors duration-150"
					>
						{token}
					</button>
				</Tooltip>
