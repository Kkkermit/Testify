...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (sub !== 'enable' && sub !== 'disable' && (!prefixSetupData || !prefixSetupData.Enabled)) {
            return await interaction.reply({ content: 'You **cannot** use this command as the prefix system has not yet been enabled. To enable the prefix system run **\`prefix enable\`**', flags: MessageFlags.Ephemeral });
        }
        ...
        switch(sub) {
            ...
            try {
                ...
                if (!data1) return interaction.reply({ content: `The prefix for ${interaction.guild.name} has not been updated and is using the default one of **\`${client.config.prefix}\`**`, flags: MessageFlags.Ephemeral });
            ...
            try {
                ...
                if (!data2) return interaction.reply({ content: `The prefix is already set to the **default!** ( \`\`${client.config.prefix}\`\` )`, flags: MessageFlags.Ephemeral });
