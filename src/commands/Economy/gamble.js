...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (timeout.includes(interaction.user.id)) return await interaction.reply({ content: "Come back in **1min** to gamble more!", flags: MessageFlags.Ephemeral });
        ...
        if (!data) return await interaction.reply({ content: "You don't have an account, create one using \`/economy-create account\`", flags: MessageFlags.Ephemeral });
            ...
            if (data.Wallet < amount) return await interaction.reply({ content: `You only have **$${data.Wallet}** in your wallet...`, flags: MessageFlags.Ephemeral });
