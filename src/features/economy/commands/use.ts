...
export default defineCommand({
	...
	async execute(ctx) {
		...
		const id = ctx.options.getString("item", true).toLowerCase().replace(/\s+/g, "_");
		...
		const consumed = await removeInventoryItem(guild.id, ctx.user.id, id, 1);
		...
		if (reward > 0) await adjustWallet(guild.id, ctx.user.id, reward);
