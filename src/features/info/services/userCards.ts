...
export function avatarEmbed(user: User): EmbedBuilder {
	...
	return embed({ category: Category.Info, title: `${user.username}'s avatar`, url, image: url });
}
...
export async function bannerEmbed(user: User): Promise<EmbedBuilder> {
	...
	return embed({ category: Category.Info, title: `${user.username}'s banner`, url, image: url });
}
