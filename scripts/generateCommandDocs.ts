...
const lines: string[] = [
	"# Commands",
	"",
	`${commands.length} commands. This file is generated by \`npm run docs:commands\` — edit the commands, not this.`,
	"",
];
...
for (const category of ALL_CATEGORIES) {
	...
	lines.push("| Command | Description | Subcommands |", "| --- | --- | --- |");

	for (const command of inCategory) {
		const subcommands =
			subcommandsOf(command)
				.map((sub) => `\`${sub.name}\``)
				.join(", ") || "—";
		lines.push(`| \`/${command.name}\` | ${command.description} | ${subcommands} |`);
	}
